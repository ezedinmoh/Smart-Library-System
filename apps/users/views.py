...
def upload_avatar(request):
    ...
    if request.method == 'POST' and request.FILES.get('profile_picture'):
        try:
            ...
            # Delete old profile picture from whatever storage is active
            if profile.profile_picture:
                try:
                    ...
                    if is_cloudinary:
                        # Delete from Cloudinary to avoid orphaned files
                        storage.delete(old_name)
                    else:
                        # Local storage — delete the file directly
            ...
            if email_messages:
                ...
                def _send():
                    ...
                    for subj, body, from_addr, to_list in _msgs:
                        try:
                            # Build HTML paragraphs from plain text
                            body_html = ''.join(
                                f'<p style="margin:0 0 12px 0;color:#374151;font-size:15px;line-height:1.6;">{line}</p>'
                                for line in body.split('\n') if line.strip()
                            )
                            html_body = render_to_string('emails/bulk_email.html', {
